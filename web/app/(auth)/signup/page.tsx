        ...
        <form onSubmit={handleSubmit} className="block w-full space-y-5">
          ...
          <div className="flex flex-col gap-3.5">
            <Button
              ...
              onClick={async () => {
                try {
                  const response = await authService.startGoogleAuth();
                  if (response.ok && response.data?.auth_url) {
                    window.location.href = response.data.auth_url;
                  } else {
                    setError(response.message || 'Failed to get Google authentication URL');
                  }
                } catch (error) {
                  console.error('Google auth error:', error);
                  setError('Failed to start Google authentication: ' + (error instanceof Error ? error.message : String(error)));
                }
              }}
            >
